...
namespace web4.Controllers
{
    public class MauInChungTuController : Controller
    {
        ...
        {
            List<MauInChungTu> dmDlist = LoadDataController.LoadData.LoadDmDt("",Request);
        ...
        public ActionResult PhieuXuatKho_Fill(MauInChungTu MauIn)
        {
            ...
            MauIn.From_date = Request.Cookies["From_date"].Value;
            MauIn.To_date = Request.Cookies["To_Date"].Value;
            string SoCt = Request.QueryString["So_Ct"];
        ...
        {
            List<MauInChungTu> dmDlist = LoadDataController.LoadData.LoadDmSalesUnit();
        ...
        public ActionResult PhieuXuatKho_Data(MauInChungTu MauIn)
        {
            ...
            Response.Cookies["From_date"].Value = MauIn.From_date.ToString();
            Response.Cookies["To_Date"].Value = MauIn.To_date.ToString();
            string Sales_Unit=Request.Cookies["Ma_Dt"].Value;
            string SoCt=Request.Cookies["So_Ct"].Value;
            MauIn.UserName = Request.Cookies["UserName"].Value;
        ...
        public ActionResult ExportBaoCaoCongNo()
        {
            ...
            var fileName = $"BangDoiChieuDoanhThuCongNo{DateTime.Now:yyyyMMddHHmmss}.xlsx";
            ...
            using (var package = new ExcelPackage())
            {
                ...
                var imagePath = Server.MapPath("~/assets/images/logo.png"); // Thay thế bằng đường dẫn thật
                                                                            // Lấy giá trị từ biến Dvcs
                string Dvcs = Request.Cookies["Dvcs"] != null ? HttpUtility.UrlDecode(Request.Cookies["Dvcs"].Value) : "";
                string NgayNo = Request.Cookies["Ngay_No"] != null ? HttpUtility.UrlDecode(Request.Cookies["Ngay_No"].Value) : "";
                ...
                worksheet.Cells["A7"].Value = $"Kính gửi khách hàng: {ten_kh}";
                worksheet.Cells["A7"].Style.Font.Bold = true;
                worksheet.Cells["A8"].Value = $"Địa chỉ khách hàng: {DiaChi}";
                worksheet.Cells["A8"].Style.Font.Bold = true;
                worksheet.Cells["A9"].Value = $"I.Số dư đến hết ngày: {NgayNo}";
                ...
                worksheet.Cells[endRowIII + 10, startColumn].Value = "(GIÁM ĐỐC/ỦY QUYỀN)";
                worksheet.Cells[endRowIII + 10, startColumn].Style.Font.Bold = true;
                worksheet.Cells[endRowIII + 11, startColumn].Value = "(Ký, ghi rõ họ tên, chức vụ, đóng dấu)";
                worksheet.Cells[endRowIII + 11, startColumn].Style.Font.Bold = true;
